...
plotsForMuller <- function(the.data, output, use.theta=TRUE, add.err.bars=FALSE, width.parameter=6, height.parameter=3, with.gene.name=TRUE, bottomline=TRUE) {
  ...
  for (snp in ordered.mutations) {
    ...
    ## Change triplicate points to mean value.
    mdata <- melt(trajectory.data, id=c("Generations"))
    cdata <- dcast(mdata,Generations~variable, mean)
    ## Add a bottom line for Steve to draw a closed polygon.
    cdata$bottom <- rep(0,length(cdata$Generations))
